...
# TODO: changed to aeon -- adds more dependencies!
#from aeon.classification.convolution_based import RocketClassifier, MultiRocketHydraClassifier
...
class ROCKAD():
    ...
    def __init__(self,
            ...
            rocket_transformer_name = "Rocket",
            random_state = 42,
        ) -> None:
        self.random_state = random_state
        ...
        self.rocket_transformer = None

        if rocket_transformer_name.lower() == "rocket":
            self.rocket_transformer = Rocket(num_kernels=self.n_kernels, n_jobs=self.n_jobs, random_state=self.random_state)
        elif rocket_transformer_name.lower() == "multirocket":
            self.rocket_transformer = MultiRocket(num_kernels=self.n_kernels, n_jobs=self.n_jobs, random_state=self.random_state)
        else:
            raise Exception('Rocket classifier in ROCKAD not set', 'ERROR: Rocket classifier')
